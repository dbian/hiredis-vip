...
{
    dictIterator *di = malloc(sizeof(dictIterator));
    dictEntry *de_f, *de_t;
                                       ...
                                       dict **nodes, cluster_node *node, sds master_name)
{
    ...
    else
    {
        ...
        else if (node->role == REDIS_ROLE_MASTER && node_old->role == REDIS_ROLE_SLAVE)
        {
            if (node->slaves == NULL)
            {
                             ...
                             const char *ip, int port)
{
    ...
    dictIterator *dit = malloc(sizeof(dictIterator));
    dictEntry *den;
    listIter *lit = NULL;
    listNode *lnode;
